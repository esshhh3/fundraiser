...
import './donor.css'; // you may rename CSS file if needed
...
export default function ViewAllCampaigns() {
  ...
  const handleBookClick = (campaignId) => {
    const donor = JSON.parse(sessionStorage.getItem("donor")); // updated
    if (!donor || !donor.id) {
      alert("Donor not logged in");
      return;
    }

    navigate(`/bookcampaign?campaignid=${campaignId}`); // updated route
  };
  ...
  return (
    <div className="event-container">
      <h3 className="event-heading">Available Campaigns</h3>
      <table className="event-table">
        <thead>
          <tr>
            <th>Campaign ID</th>
            <th>Creator Company Name</th>
